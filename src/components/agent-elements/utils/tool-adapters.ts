...
export function mapToolInvocationToStep(
        ...
        const stdout =
          typeof resultRecord.stdout === "string"
            ...
            : typeof resultRecord.output === "string"
              ...
              : "";
        const stderr =
          typeof resultRecord.stderr === "string" ? resultRecord.stderr : "";
  ...
  const resultRecord =
    typeof result === "object" && result !== null
      ? (result as Record<string, unknown>)
      : undefined;
  if (toolName === "Edit" && Array.isArray(resultRecord?.structuredPatch)) {
    const patches = resultRecord.structuredPatch.filter(
      (patch): patch is { lines?: string[] } =>
