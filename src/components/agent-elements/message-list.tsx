...
export const MessageList = memo(function MessageList({
  ...
  const [initialLastMessageId] = useState(
    () => messages[messages.length - 1]?.id ?? null,
  );
  const [initialScrollBehaviorValue] = useState(initialScrollBehavior);
  ...
  useLayoutEffect(() => {
    ...
    if (initialScrollBehaviorValue === "top") {
  ...
  const showAssistantBreathingSpace = showPlanning || isNewAssistantMessage;
