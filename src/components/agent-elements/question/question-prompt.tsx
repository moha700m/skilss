...
export function QuestionPrompt(props: QuestionPromptProps) {
  ...
  const totalQuestions = props.totalQuestions ?? props.questions.length;
  const clampedIndex = Math.max(1, Math.min(questionIndex, totalQuestions));
  const activeQuestion = props.questions[clampedIndex - 1];
  const resetKey = JSON.stringify([
    clampedIndex,
    activeQuestion?.kind,
    activeQuestion?.allowCustom ?? false,
    props.initialAnswer ?? null,
  ]);

  return <QuestionPromptStateful key={resetKey} {...props} />;
}

function QuestionPromptStateful({
  questions,
  questionIndex = 1,
  totalQuestions,
  onPreviousQuestion,
  ...
  const initialSelectedIds = (() => {
    ...
    if (activeQuestion?.kind === "text") return [];

    const selected = new Set(initialAnswer.selectedIds ?? []);
    if (customEnabled && (initialAnswer.text?.trim().length ?? 0) > 0) {
  ...
  const [customText, setCustomText] = useState(
    activeQuestion?.kind === "text" ? "" : (initialAnswer?.text ?? ""),
  );
